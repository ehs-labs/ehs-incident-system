...
Handlers::DomainEvent.register('IncidentSubmitted') do |event|
  ...
  Handlers::IncidentNotifier.notify(
    ...
    body: "#{Handlers::DomainEvent.actor_name(event)} reported a new incident at severity #{event.dig('subject',
                                                                                                      'severity')}: #{event.dig(
                                                                                                        'subject', 'summary'
                                                                                                      )}",
...
Handlers::DomainEvent.register('IncidentAssigned') do |event|
  ...
  Handlers::IncidentNotifier.notify(
    ...
    body: "#{Handlers::DomainEvent.actor_name(event)} assigned incident ##{incident_id} (severity #{event.dig(
      'subject', 'severity'
...
Handlers::DomainEvent.register('IncidentClosed') do |event|
  ...
  Handlers::IncidentNotifier.notify(
    ...
    body: "Your incident ##{incident_id} (severity #{event.dig('subject',
                                                               'severity')}) has been resolved and closed by #{Handlers::DomainEvent.actor_name(event)}.",
    link_path: "/incidents/#{incident_id}"
  )
end
...
Handlers::DomainEvent.register('SlaBreached') do |event|
  ...
  Handlers::IncidentNotifier.notify(
    ...
    body: "Incident ##{incident_id} (severity #{event.dig('subject',
                                                          'severity')}) is past its #{event.dig('subject',
                                                                                                'sla_kind').downcase} SLA window and still needs action.",
    link_path: "/incidents/#{incident_id}"
  )
end
